...
import com.betterman.util.DateHelp;
import com.betterman.util.RGBLuminanceSource;
import com.betterman.util.SDCardHelp;
...
public class ImageActivity extends Activity implements OnClickListener {
	...
	private void encodeQRCode() {
		try {
			...
			/*
			 * 第一个参数：输入的文本 第二个参数：条形码样式－》二维码 第三个参数：宽度 第四个参数：高度 第五个参数：map保存编码类型
			 */
			...
			/*
			 * 第一个参数：填充位图的像素数组 第二个参数：第一个颜色跳过几个像素读取 第三个参数：像素的幅度 第四个参数：起点x坐标
			 * 第五个参数：起点y坐标 第六个参数：宽 第七个参数：高
			 */
			bitmap.setPixels(pixels, 0, QR_WIDTH, 0, 0, QR_WIDTH, QR_HEIGHT);
			// 显示图片
			qr_image.setImageBitmap(bitmap);
			String filename = DateHelp.getFormateDate(1, new Date()) + ".jpg";
			String filepath = "/sdcard/2weima/";
	...
	public void saveFile(Bitmap bitmap, String filename, String path) {
		if (!SDCardHelp.getFileSavePath(path).equals(null)) {
			File file = new File(SDCardHelp.getFileSavePath(path), filename); // 创建图片文件
